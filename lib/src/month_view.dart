...
class _MonthViewState extends State<_MonthView> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _chevronOpacityAnimation = widget.headerStyle.enableFadeTransition
        ...
        : _chevronOpacityController.drive(Tween<double>(begin: 1.0, end: 1.0).chain(CurveTween(curve: Curves.easeInOut)));
  }
  ...
  void _handleNextMonth() {
    if (!_isDisplayingLastMonth) {
      SemanticsService.announce("${formattedMonth(_nextMonthDate!.month, Language.english)} ${_nextMonthDate!.year}", textDirection);
  ...
  void _handlePreviousMonth() {
    if (!_isDisplayingFirstMonth) {
      SemanticsService.announce("${formattedMonth(_previousMonthDate!.month, Language.english)} ${_previousMonthDate!.year}", textDirection);
